...
def train(data_folder=".", model_folder="runs"):
    ...
    keys = ("image", "label")
    #percentage of images used for training and testing
    train_frac, val_frac = 0.75, 0.25
    ...
    #run 1
    #max_epochs, lr, momentum = 15, 1e-4, 0.95
    #run 2
    #max_epochs, lr, momentum = 15, 1e-2, 0.7
    # run 3
    #max_epochs, lr, momentum = 15, 1e-2, 0.7
    # run 4
    #max_epochs, lr, momentum = 15, 1e-2, 0.7
    # run 5
    max_epochs, lr, momentum = 15, 1e-4, 0.5
    #event handler, displays the total number of epochs,
    logging.info(f"epochs {max_epochs}, lr {lr}, momentum {momentum}, device {device}  netP {net.parameters()}")

    #Adam optimizer run 1
    #opt = torch.optim.Adam(net.parameters(), lr=lr)
    # ASGD optimizer run 2
    #opt = torch.optim.ASGD(net.parameters(), lr=lr)
    # ASGD optimizer run 2.5    No Comvolution
    #opt = torch.optim.RMSprop(net.parameters(), lr=lr)
    # LBFGS optimizer run 2.5   No Comvolution
    #opt = torch.optim.LBFGS(net.parameters(), lr=lr)
    # Adagrad optimizer run 3
    #opt = torch.optim.Adagrad(net.parameters(), lr=lr)
    #  optimizer run 4
    #opt = torch.optim.RMSprop(net.parameters(), lr=lr)
    #  optimizer run 5
    opt = torch.optim.Rprop(net.parameters(), lr=lr)
